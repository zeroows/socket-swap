...
impl Router {
    ...
    pub async fn route(&self, req: Request<Incoming>) -> Result<Response<BoxBody>, Error> {
        ...
        let response = match (method, path.as_str()) {
            // Info endpoints
            (Method::GET, "/_ping") => handle_ping()
                .await
                .map_err(|_| Error::Internal("Infallible".to_string()))
            ...
            (Method::DELETE, path) if path.starts_with("/containers/") => {
                ...
                handle_delete(container_id, self.job_manager.clone())
                    .await
                    .map(|r| r.map(box_body))?
            }
            ...
            (Method::GET, path) if path.starts_with("/containers/") && path.ends_with("/json") => {
                ...
                handle_inspect(container_id, self.job_manager.clone())
                    .await
                    .map(|r| r.map(box_body))?
            }
