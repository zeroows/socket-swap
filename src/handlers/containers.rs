use crate::docker::{
    ...
    ContainerState, ContainerConfig, ErrorResponse, ContainerWaitResponse, ErrorDetail,
};
...
pub async fn handle_create(
    ...
    let labels = create_req.labels.into_iter().collect();
    ...
    job_manager
        .create_job(
            ...
            create_req.env,
            create_req.cmd,
            create_req.entrypoint,
            create_req.working_dir,
            labels,
        )
        .await?;
...
pub async fn handle_inspect(
    ...
    let job_name = job.metadata.name.as_ref().unwrap_or(&"unknown".to_string()).clone();
    let created = job.metadata.creation_timestamp
        .as_ref()
        .map(|t| t.0.to_rfc3339())
        .unwrap_or_else(|| "unknown".to_string());
    
    // Get container spec from job
    let container_spec = job.spec
        .as_ref()
    ...
    let (status, running, exit_code, started_at, finished_at) = if let Some(job_status) = &job.status {
        if job_status.succeeded.unwrap_or(0) > 0 {
            ("exited".to_string(), false, 0, 
             job_status.start_time.as_ref().map(|t| t.0.to_rfc3339()).unwrap_or_default(),
             job_status.completion_time.as_ref().map(|t| t.0.to_rfc3339()).unwrap_or_default())
        } else if job_status.failed.unwrap_or(0) > 0 {
            ("exited".to_string(), false, 1,
             job_status.start_time.as_ref().map(|t| t.0.to_rfc3339()).unwrap_or_default(),
             job_status.completion_time.as_ref().map(|t| t.0.to_rfc3339()).unwrap_or_default())
        } else if job_status.active.unwrap_or(0) > 0 {
            ("running".to_string(), true, 0,
             job_status.start_time.as_ref().map(|t| t.0.to_rfc3339()).unwrap_or_default(),
             "".to_string())
        } else {
            ("created".to_string(), false, 0, "".to_string(), "".to_string())
        }
    } else {
        ("created".to_string(), false, 0, "".to_string(), "".to_string())
    };
